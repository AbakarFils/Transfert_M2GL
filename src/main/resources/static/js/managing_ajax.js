...
(function ($) {
    ...
    $.fn.managing_ajax = function (args) {
        ...
        if ($('#' + id_table).length != 0)
            table = $('#' + id_table).DataTable({
                ...
                columnDefs: args.columnDefs == undefined ? [{
                    orderable: false,
                    targets: -1
                }] : args.columnDefs,
        ...
        if ($('#btn_add').length != 0 || $('.btn_add').length != 0)
            $('#btn_add,.btn_add').click(function (e) {
                ...
                $('#' + args.id_form).find('input[type=text],input[type=hidden],textarea').val('');
            ...
            $('#' + id_table + ' tbody').on('click', '.btn_show', function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('#' + args.id_form + " input, #" + args.id_form + " select, #" + args.id_form + ' textarea').prop('disabled', true);
                            ...
                            success: function (data) {
                                ...
                                if (data.status == 'success') {
                                    ...
                                    if (args.link_reload != undefined)
                                        ...
                                        var lhref = $('#' + menu_clicked).attr('link');
            ...
            $('#' + args.id_form).submit(function (e) {
                ...
                if (isvalidate === true) {
                    ...
                    if ($('#' + args.id_form).attr('enctype') == 'multipart/form-data') {
                        ...
                        var form = $('#' + args.id_form).serialize();
                        $('#' + args.id_form + ' input[disabled] textarea[disabled] select[disabled]').each(function () {
        ...
        if ($('#' + args.id_modal_form).length != 0)
            $('#' + args.id_modal_form).on('hidden.bs.modal', function (event) {
                ...
                if (show_action == true) {
                    ...
                    $('#' + args.id_form + " input, #" + args.id_form + " select, #" + args.id_form + " textarea").prop('disabled', false);
                    show_action = false;
                    return false;
                }
    ...
    $.fn.divcrud = function (args) {
        ...
        if ($('#' + args.id_form).length != 0) {
            ...
            /*
             $(".numeric-digit-sep-mount").on("keypress keyup blur",function (event) {
                $(this).val($(this).val().replace(/[^\d].+/, ""));
                if ((event.which < 48 || event.which > 57)) {
                    event.preventDefault();
                }

				var n = $(this).val().toString(), p = n.indexOf('.');
